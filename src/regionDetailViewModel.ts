...
export function createRegionDetailViewModel(region: Region, prefecture: Prefecture, siblings: Region[], dialects: Dialect[], prefectureDialects: Dialect[] = dialects, cultureItems: RegionalCultureItem[] = []): RegionDetailViewModel {
  ...
  return {
    ...
    featuredWords: ranked.slice(0, 6).map((item) => ({ id: item.id, word: item.phrase, reading: hasEvidenceScope(item, "reading") ? text(item.reading) : undefined, meaning: item.standardJapanese, municipality: text(item.municipality), verificationLabel: confirmed(item) ? "資料確認" : "確認継続中" })),
