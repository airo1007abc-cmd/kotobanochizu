          ...
          <section className="v2-content-card">
            ...
            <section className="v2-section-block v2-source-card">
              ...
              <div className="v2-source-record">
                ...
                {vm.sources.length ? <div className="v2-source-list">{vm.sources.map((source, index) => <div key={`${source.url ?? source.title ?? "source"}-${index}`}>{source.url ? <a href={source.url} target="_blank" rel="noreferrer">{source.title ?? "出典資料"}<ArrowRight /></a> : source.title ?? "資料確認中"}<SourceAvailability url={source.url} />{source.organization && <small>{source.organization}</small>}{source.page && <small>掲載箇所：{source.page}</small>}{source.checkedAt && <small>資料確認日：<time dateTime={source.checkedAt}>{source.checkedAt}</time></small>}{source.checkedFields.length > 0 && <small>この資料で確認：{source.checkedFields.join('・')}</small>}{source.recordingYear && <small>記録年：{source.recordingYear}年</small>}{source.note && <details><summary>掲載箇所・資料の注記</summary><p>{source.note}</p></details>}</div>)}</div> : <p>資料確認中</p>}
              </div>
