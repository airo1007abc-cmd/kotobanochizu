...
import { Archive, BookOpen, ChevronDown, ExternalLink, Film, MessageCircle, Music2, Volume2 } from "lucide-react";
...
export function RegionalCultureSection({ title, items }: { title: string; items: RegionalCultureItem[] }) {
  ...
  return <section className="regional-culture" aria-labelledby="regional-culture-title">
    ...
    <div className="regional-culture-grid">{visibleItems.map((item) => { const Icon = icons[item.type]; return <a href={item.sourceUrl} target="_blank" rel="noreferrer" key={item.id}>
    ...
    {items.length > 8 && <button className="regional-culture-more" type="button" aria-expanded={showAll} onClick={() => setShowAll((value) => !value)}>
      {showAll ? "表示を戻す" : `文化資料をさらに${items.length - 8}件見る`} <ChevronDown aria-hidden="true" />
    </button>}
