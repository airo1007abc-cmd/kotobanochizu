...
type ContextRecord = {
  ...
  relatedComparison?: { slug: string; title: string; description: string };
};
...
export function ContextGuide() {
  ...
  return <article className="region-guide-page context-guide-page">
    ...
    <section><h2>発話と標準語訳</h2><div className="utterance-list">{dialects.map((item, index) => <article key={item.id}><small>{String(index + 1).padStart(2,"0")} / {hasEvidenceScope(item, "usage") ? item.usageContexts.join("・") : "使用場面未確認"}</small><blockquote>{hasEvidenceScope(item, "example") ? item.exampleDialect : "用例は確認中です"}</blockquote><p>{hasEvidenceScope(item, "example") ? item.exampleStandard : "出典で確認できる用例を確認中です。"}</p><Link to={`/dialects/${item.id}`}><strong>{item.phrase}</strong>の意味・地域・出典を確認する<ArrowRight /></Link></article>)}</div></section>
    {record.relatedComparison && <section className="guide-introduction" aria-labelledby="context-related-comparison"><h2 id="context-related-comparison">別の地域と比べて読む</h2><p>{record.relatedComparison.description}</p><Link to={`/meanings/${record.relatedComparison.slug}`}>{record.relatedComparison.title}<ArrowRight aria-hidden="true" /></Link></section>}
