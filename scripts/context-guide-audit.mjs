import { hasEvidenceScope, isIndexableRecord } from "../src/evidencePolicy.mjs";
...
const report={generatedAt:new Date().toISOString(),total:records.length,indexable:records.filter((item)=>item.indexStatus==="indexable").length,exampleCounts:Object.fromEntries(records.map((item)=>[item.id,item.dialectIds.length])),invariants:{indexableGuidesWithMissingExampleEvidence},duplicateTitles:duplicate("title"),duplicateDescriptions:duplicate("description"),duplicateSearchIntents:duplicate("searchIntent"),failures};
